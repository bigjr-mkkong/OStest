...
void init_mem(){
	unsigned long tot=0 ;
	/* tot have multiple meaning in the code, now is as the size of useable memory*/
	struct int15_e820 *p=NULL;
	printk(WHITE,BLACK,"Display Physics Memory Distribution\n");
	p=(struct int15_e820*)0xffff800000007e00;

/*
go throgh all the information generated by the int 15h with the function number e820h
save these info in struct int15_e820 
*/
	for(int i=0;i<32;i++){
...
/*
tot now is the number of useable page number(page size=2M)
*/
	tot=0;
...
/*------------------------init zones in mm------------------------------------------------*/
	...
	for(int i=0;i<mman_struct.e820_length;i++){
	//go through every mm segment in e820 and initialize the struct zone in mman_struct
...
//go through all memory zones in mman_struct
	int end_zone_num;
